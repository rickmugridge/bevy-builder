...
use bevy::prelude::{Commands, Component, Display, Event, EventReader, Interaction, Node, Query, World};
...
fn open_close(mut commands: Commands, query: Query<(&Interaction, &mut OpenClose)>) {
    println!("Open close");
...
fn open_close_reactor(
    ...
    for OpenCloseChanged{destination_id,  open} in events.read() {
